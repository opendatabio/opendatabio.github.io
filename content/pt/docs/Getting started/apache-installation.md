...
apt-get install apache2 mysql-server php7.4 libapache2-mod-php7.4 php7.4-mysql \
		php7.4-cli pandoc php7.4-mbstring php7.4-xml php7.4-gd php7.4-pcntl php7.4-bcmath \
		supervisor
     \
a2enmod php7.4
phpenmod mbstring
...
php -m | grep -E 'mbstring|cli|xml|gd|mysql|pandoc|supervisord|bcmath|pcntl'
```
Habilite os módulos Apache 'mod_rewrite' e 'mod_alias'. Adicione o seguinte à sua configuração do Apache.
...
Configure seu arquivo **php.ini**. O instalador pode reclamar da falta de extensões do PHP, então lembre-se de ativá-las nos arquivos **cli** e na web **ini** para PHP! Atualize os valores para as seguintes variáveis:
...
Lembre-se de reiniciar o servidor Apache após editar os arquivos. No Ubuntu seria:
...
1. Crie um arquivo `.env` com o conteúdo do` cp .env.example .env` fornecido
1. Leia os comentários neste arquivo e ajuste de acordo

Entre no diretório raiz da instalação (aqui, `/home/odbserver/opendatabio`) e execute o instalador:
...
Se o script de instalação for concluído com sucesso, você está pronto para prosseguir! Aponte seu navegador para http://localhost/. As migrações de banco de dados incluem uma conta de administrador, com login `admin@example.org` e senha` password1`. Altere a senha após a instalação.
